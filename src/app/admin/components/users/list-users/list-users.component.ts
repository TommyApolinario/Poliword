...
export class ListUsersComponent {
  ...
  approveUser(userID: number){
    this.sweetAlerts.alertConfirmCancelQuestion("Activar usuario", "¿Estás seguro de querer activar este usuario para admitirlo en Poliword? Aprobar al usuario hará que pueda realizar todas las funciones que están disponibles en esta aplicación, según su perfil de usuario.").then(respuesta => {
      if (respuesta.value) {
        this.spinnerStatus = false;
  ...
  desactivateUser(userID: number){
    this.sweetAlerts.alertConfirmCancelQuestion("Desactivar usuario", "¿Estás seguro de querer desactivar este usuario? Lo que se hará es bloquear al usuario para que ya no pueda acceder a las funciones de Poliword").then(respuesta => {
      if (respuesta.value) {
        this.spinnerStatus = false;
